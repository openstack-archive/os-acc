...
def get_config(instance, inst_type, acc):
    ...
    conf = plugin.get_config(instance, inst_type, acc)
    return conf


def attach(acc, instance):
    if _EXT_MANAGER is None:
        raise os_acc.exception.LibraryNotInitialized()
    acc_type = acc['type']
    if acc_type is None:
